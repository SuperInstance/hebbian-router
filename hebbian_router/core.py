...
@dataclass
class Route:
    """A pathway between a source and destination node.

    Strength grows with use and successful reception. Weakens with disuse
    or failure. Never reaches exactly 0 or 1 — chaos keeps exploration alive.
    """
    source: str
    destination: str
    strength: float = 0.5
    ...
    def fire(self, chaos: float = 0.1) -> bool:
        """Attempt to fire this route (scalar path — simple, for single calls)."""
        self.fires += 1
...
class RoutingLayer:
    """Manages routes and Hebbian channels between nodes.
    ...
    **Performance**: Uses precomputed indexes and vectorized ``fire_fast()``.
    ...
    Args:
        chaos: Probability of stochastic exploration (default 0.1).
    ...
    def fire(self, source: str, destinations: Optional[list[str]] = None) -> list[str]:
        ...
        Use :meth:`fire_fast` for bulk operations (60× faster).
        """
        with self._lock:
            candidates = [
    ...
    def fire_fast(
        ...
        """Vectorized route firing — 60× faster than :meth:`fire`.
        ...
        dests = [None] * n  # type: ignore[list-item]
